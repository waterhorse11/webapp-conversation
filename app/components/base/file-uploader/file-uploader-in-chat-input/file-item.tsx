...
import { ReplayLine } from '@/app/components/base/icons/other'
...
const FileItem = ({
  ...
  return (
    <>
      <div
        className={cn(
          'bg-white group/file-item relative h-[68px] w-[144px] rounded-lg border-[0.5px] border-components-panel-border p-2 shadow-xs',
      ...
      >
        {
          showDeleteAction && (
            <Button
              className='!absolute !-right-1.5 !-top-1.5 !z-[11] !hidden !h-6 !w-6 !rounded-full !p-0 group-hover/file-item:!flex !bg-gray-100 hover:!bg-gray-200 !px-0'
              type='primary'
              onClick={() => onRemove?.(id)}
            >
              <RiCloseLine className='h-4 w-4 text-gray-600' />
            </Button>
          )
        }
        <div
          className='system-xs-medium mb-1 line-clamp-2 h-8 cursor-pointer break-all text-gray-600'
          title={name}
        ...
        <div className='relative flex items-center justify-between'>
          <div className='system-2xs-medium-uppercase flex items-center text-gray-600'>
            <FileTypeIcon
              size='sm'
          ...
          </div>
          {
            showDownloadAction && tmp_preview_url && (
              <ActionButton
                size='m'
                className='absolute -right-1 -top-1 hidden group-hover/file-item:flex hover:bg-gray-100 hover:rounded-lg p-1'
                onClick={(e) => {
                  e.stopPropagation()
                  downloadFile(tmp_preview_url || '', name)
                }}
              >
                <RiDownloadLine className='h-4 w-4 text-gray-600' />
              </ActionButton>
            )
          }
          {

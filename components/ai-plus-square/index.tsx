...
// 应用卡片组件
const KimiPlusCard: FC<AppCardProps> = ({ icon, titleKey, descriptionKey, href, isPinned, onTogglePin, id, onShowTogglePinApp }) => {
...
const AiPlus: FC<AiPlusProps> = ({ onShowTogglePinApp }) => {
    const { t } = useTranslation()
    // 导航项
    const navItems = APP_GROUPS.map(group => ({
        key: group.titleKey,
        title: t(group.titleKey) || group.titleKey
    }))
    ...
    const initializeAppGroups = () => {
        ...
        let initialGroups = APP_GROUPS.map(group => ({
        ...
        if (savedPinnedApps) {
            try {
                ...
                const allApps = initialGroups.flatMap(group =>
                    group.title !== t('app.apps.groups.pinned') ? group.apps : []
                );
    ...
    const [appGroups, setAppGroups] = useState(initializeAppGroups)
        ...
        <div className="square-container rounded-lg">
            <div className="layout-container">
                ...
                <div className="square">
                    <div className="square-title">{t('app.apps.title')}</div>
                    <ul className="square-nav">
                        {navItems.map((item) => (
                            <li
                                key={item.key}
                                className={`square-nav-item ${activeNav === item.title ? 'active' : ''}`}
                    ...
                    <div className="square-list">
                        {activeNav === navItems[0].title ? (
                            <>
                                {appGroups.map((group) => (
                                    <div key={group.titleKey} className="square-group">

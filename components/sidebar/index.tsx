...
const Sidebar: FC<ISidebarProps> = ({
  ...
  const handleConversationClick = (appId: string, id: string) => {
    if (id === '-1') {
      router.push(`/`, { scroll: false })
    } else {
      router.push(`/chat/${appId}/${id}`, { scroll: false })
  ...
  return (
    <>
          ...
          <div className="mt-4">
            <div className="ai-plus-part">
              ...
              <div className="mt-0 space-y-0.5">
                        ...
                        <a onClick={() => {
                          ...
                          <div className="ai-plus-info flex items-center flex-1">
                            <div className="ai-plus-avatar w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center mr-1.5 overflow-hidden">
                              <img src={app.icon} alt={t(app.titleKey) || app.titleKey} className="w-5 h-5 rounded-full object-cover" />
                            </div>
                            <span className="text-sm font-normoal">{t(app.titleKey) || app.titleKey}</span>
                          </div>
                        ...
                        {openMenuId === app.id && menuPosition && (
                          ...
                          >
                            ...
                            >
                              ...
                              <span>{t('app.apps.operations.newChat')}</span>
                            </button>
                            <button
                            ...
                            >
                              ...
                              <span>{t('app.apps.operations.unpin')}</span>
                            </button>
                          </div>
                        )}
                      </div>
                ...
                }, [t, openMenuId, menuPosition, selectedAppId, showNewPinApp])}
              </div>
            </div>
            ...
            <div className="history-part mt-4">
              <div className="nav-title flex items-center px-2 mb-2">
                <div className="title-label flex items-center text-gray-900">
                  ...
                  <span className="text-sm">{t('common.conversation.history')}</span>
                </div>
              </div>
              <nav className="mt-4 flex-1 space-y-1 pl-4">
                <div className="max-h-[calc(90vh-20rem)] overflow-y-auto">
                  {list.slice(0, showAllConversations ? undefined : 6).map((item) => {
                    ...
                    return (
                      <div
                        onClick={async () => {
                          handleConversationClick(item.appId, item.id)
                        }}
                        key={item.id}
                ...
                {list.length > 6 && (
                  <div
                    className="flex items-center justify-center text-sm text-gray-500 hover:text-gray-700 cursor-pointer"
                    onClick={() => setShowAllConversations(!showAllConversations)}
                  >
                    {showAllConversations ? t('common.conversation.collapse') : `${t('common.conversation.viewAll')} (${list.length})`}
                  </div>
                )}
              </nav>
            </div>
      ...
      <Modal
        ...
        title={t('common.conversation.rename')}
      ...
      <Modal
        ...
        title={t('common.conversation.delete')}
      ...
      >
        <p className="text-sm text-gray-500">{t('common.conversation.confirmDelete')}</p>
      </Modal>
    </>
  )
}
